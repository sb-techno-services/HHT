...
if($_GET['mid'] > 0) {
	...
	$res=mysqli_fetch_assoc($qry);
			...
			$uid=$res['e_id'];
			$_SESSION['usrweb_mano'] = $res['email'];
			$_SESSION['usrweb_manoname'] = $res['e_fname'];

			#insert Order information
			$order_no=rand(10000,100000);
			$total_amt=$cart->get_cart_total();
			 $sql="insert into orders (ORDER_NO,ORDER_DATE,UID,TOTAL_AMT,PAY_STATUS) values ('{$order_no}',NOW(),'{$uid}','{$total_amt}','1')"; //exit();
			if($con->query($sql)){
				$oid=$con->insert_id;
	...
	if(isset($_POST["save"])){
		...
		$userquery=$con->query($sql);
			...
			$_SESSION['usrweb_mano'] = addslashes(trim($_POST['email']));
			$_SESSION['usrweb_manoname'] = addslashes(trim($_POST['fname']));
