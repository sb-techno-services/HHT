...
$qry_account = mysqli_query($conn,"SELECT * FROM hht_events ");
$account = mysqli_fetch_assoc($qry_account);

if($_GET['j_id']){
	$qry=mysqli_query($conn,"select ja_id from hht_events WHERE ja_id=".$_REQUEST['j_id']."");

	$res=mysqli_fetch_assoc($qry);
	if(mysqli_num_rows($qry)>0){
		$del=mysqli_query($conn,"delete from hht_events WHERE ja_id=".$_REQUEST['j_id']."");
...
<title>:: <?php echo $DOMAIN_NAME_PAGE.' Events Details';?> ::</title>
  ...
  <div id="wrapper">
    ...
    <section>
      ...
      <?php //include_once("employee_detials_left.php"); ?>
		...
		?>
        <div class="btn"><a href="add_event.php">Add Event </a></div>
          <table width="130%" border="0"  cellpadding="0" cellspacing="0">
            <tbody>
           
              <tr>
                <th width="8%" align="center" valign="middle" bgcolor="#d8d1e9">S.No.</th>
                <th width="12%" align="center" valign="middle" bgcolor="#d8d1e9">Event Title</th>
                <th width="21%" align="center" valign="middle" bgcolor="#d8d1e9">Event Start Date</span></th>
			...
			?>
              <tr class="<?=$class?>">
                ...
                <td align="left"><?=$arrUsersRow['event_title']?></td>
                <td align="left"><?php if($arrUsersRow['event_sdate']!=''){echo date('d/m/Y',strtotime($arrUsersRow['event_sdate']));}?></td>
                <td align="left"><a href="event_view.php?j_id=<?=$arrUsersRow['ja_id']?>">View</a> | <a href="edit_event.php?j_id=<?=$arrUsersRow['ja_id']?>">Edit</a> | <a href="events.php?j_id=<?=$arrUsersRow['ja_id']?>">Delete</a></td>
              </tr>
              <?php				  	
